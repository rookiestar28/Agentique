...
Current status: source repository, schema, documentation, package registry pages, action usage reference, badge/readback documentation, and existing `agentique.io` public links are approved for advertising after publication and smoke testing. The uploader package page is approved after owner-approved publication, registry readback, and clean install smoke. Current catalog/download package changes are included in the coordinated `0.2.0` package release after the scoped package-release gate passed. Registry readback currently shows schemas, validator, action, readback, and uploader at `0.2.0`. Direct-download live availability is not advertised until owner-approved direct-download evidence passes.
