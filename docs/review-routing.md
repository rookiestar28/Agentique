...
| Readback | `packages/readback/` | Read-only methods only, source-branch parser/variant summaries, stale/unavailable states, trust projection wording, no safety guarantee wording |
| Uploader | `packages/uploader/` | Review-only wording, auth redaction, token handling, checkpoint readiness, source-branch import-plan and variant-plan dry-runs, local draft and patch output, no platform publication or approval claims |
